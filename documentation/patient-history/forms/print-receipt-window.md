# Print Receipt Window (კალმის აიქონს რო ვაჭერთ რო იხსნება)
...
## Section 3: დახდვები (Payments)

**Section Toggle**: ☑ Checkbox (Currently CHECKED)

This section contains TWO identical sub-sections:
1. **პირველი მწლობივილი** (First Payment Method)
2. **მეორე მწლობივილი** (Second Payment Method)

### Sub-section: პირველი მწლობივილი (First Payment Method)

| Field ID | Label (Georgian) | Label (English) | Type | Required | Validation | Default Value | Notes |
|----------|------------------|-----------------|------|----------|------------|---------------|-------|
| TBD | კომპანია | Company | select | No | Must select from list | იმედი L | Insurance/payment company |
| TBD | ტიპი | Type | select | No | Must select from list | (empty) | Payment type |
| TBD | პოლისის # | Policy # | text | No | Text/Numeric | (empty) | Insurance policy number |
| TBD | მიმართვის # | Reference # | text | No | Text/Numeric | (empty) | Reference/claim number |
| TBD | გადევნის თარიღი | Payment Date | date | No | Valid date | (empty) | Date of payment |
| TBD | მოქმედების ვადა | Validity Period | date | No | Valid date | (empty) | Policy validity period |
| TBD | თანაგადახდის % | Co-payment % | number | No | 0-100 | 0 | Co-payment percentage |

**Company (კომპანია) - Currently Selected**: იმედი L (Imedi L)

### Sub-section: მეორე მწლობივილი (Second Payment Method)

| Field ID | Label (Georgian) | Label (English) | Type | Required | Validation | Default Value | Notes |
|----------|------------------|-----------------|------|----------|------------|---------------|-------|
| TBD | კომპანია | Company | select | No | Must select from list | (empty) | Insurance/payment company |
| TBD | ტიპი | Type | select | No | Must select from list | (empty) | Payment type |
| TBD | პოლისის # | Policy # | text | No | Text/Numeric | (empty) | Insurance policy number |
| TBD | მიმართვის # | Reference # | text | No | Text/Numeric | (empty) | Reference/claim number |
| TBD | გადევნის თარიღი | Payment Date | date | No | Valid date | (empty) | Date of payment |
...
- The entire "დახდვები" (Payments) section appears/disappears based on the checkbox state
- When checkbox is CHECKED (☑): Both payment method sub-sections are visible
- When checkbox is UNCHECKED (☐): Payment sections are hidden
