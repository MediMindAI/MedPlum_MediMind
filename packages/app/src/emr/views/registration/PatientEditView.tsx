...
/**
 * Patient Edit View
 * Full-page edit route at /emr/registration/edit/:id
 * Loads patient data by ID and displays editable form
 */
export function PatientEditView(): JSX.Element {
  ...
  const loadPatient = async () => {
    try {
      setLoading(true);
      const data = await medplum.readResource('Patient', id as string);
      setPatient(data);
      ...
      navigate('/emr/registration/registration');
    } finally {
      setLoading(false);
    }
  };
            ...
            <Grid mt="md">
              <Grid.Col span={6}>
                <Button
                  variant="outline"
                  fullWidth
                  onClick={() => navigate('/emr/registration/registration')}
                  disabled={submitting}
                >
