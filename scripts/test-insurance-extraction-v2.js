/**
 * Quick Test V2 - Insurance Table Extraction (4 OR 5 columns)
...
(function testInsuranceExtraction() {
  ...
  tables.forEach((table, index) => {
    ...
    // UPDATED: Accept 4 OR 5 columns (5th is action column)
      ...
      if (firstCell && firstCell.length > 3 && !/^\d+$/.test(firstCell)) {
        ...
        for (let i = 0; i < Math.min(3, rowCount); i++) {
          ...
          insuranceData.push({
            ...
            action: cells[4] ? 'has action column' : 'no action column'
          });
        }
      } else {
  ...
  if (foundInsuranceTable) {
    ...
    insuranceData.forEach((item, i) => {
      ...
      console.log(`     ${item.date} | ${item.price} ${item.currency} | ${item.action}`);
    });
  } else {
